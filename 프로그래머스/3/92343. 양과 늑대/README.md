...
- 처음에는 DFS + 완탐 vs BFS + 우선순위큐 중에 고민하다가 전자를 선택헀는데 테케가 2개중에 하나만 안풀리더라 그래서 고민하다가 우선순위큐로 푸는 풀이가 잘못된 것 같다는 느낌이 들었고
- 질문 게시판에서 DFS로 푸는 힌트를 얻어 풀어보려 했지만, 내가 아는 DFS는 끝까지 탐색하는 것인데 이 문제는 왔다갔다 해야하는 경우라 감이 안잡혀서 블로그에 있는 코드를 보고 이해했다...

- DFS인데 갈 수 있는 노드에 대해서 List에 저장해두었다가 순회하면서 DFS를 하는 것이다.
- 이런 풀이는 처음 봤는데,, 다음에 비슷한 문제가 나오면 맞출 수 있도록 해야겠고, 유연하게 생각할 수 있도록 해야겠다 ㅠㅜ
