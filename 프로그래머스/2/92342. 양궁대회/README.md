...
뭔가 규칙이 있을줄 알고 계속 잡고 있었던 게 시간을 지체시키지 않았나 싶음
앞으로 규칙이 없으면 완탐이나 더 좋은 방법을 생각해봐야 겠다.

그리고 점수 차의 최대값이 같을 때 낮은 점수를 판별하는 부분에서 낮은 인덱스를 찾아야 한다는 강박?에 휩쓸려 시간을 잡아 먹었는데
그냥 10번 index부터 하나씩 비교하면 되는 간단한 문제였따...
